...
class CategoryService:
    ...
    def create_category(data: CategoryCreateData) -> Category:
        """
        Create a new category in transaction.
        and locks parent row 
        """
    ...
    def delete_category(category_id: int) -> None:
        ...
        # we can remove select_for_update if there is not additional logic
        category.delete()
    ...
    def build_full_tree() -> List[dict]:
        ...
        nodes: Dict[int, dict] = {} # create  dict for nodes/categories
        children_map: Dict[Optional[int], List[int]] = {} # creat second dict to keep chidrens only 

        for c in categories: # iterate over all  categories
            nodes[c.id] = { 
                "id": c.id,
                "name": c.name,
                ...
                "children": [],
            } # add all categories with its data
            children_map.setdefault(c.parent_id, []).append(c.id) # add all categories with its parent

        def attach_children(parent_id: Optional[int]) -> List[dict]:
            items: List[dict] = []
            for child_id in children_map.get(parent_id, []): # get all childrens for this parent
                node = nodes[child_id] # add all childrens to it's parent nested
